...
# Create single-suite descriptor with cache disabled. Also inject a
# `mkdir -p src/obj` between ./configure and make: v6.12.5 added
# -Werror -Wmissing-include-dirs in src/Makefile.am, which makes the
# parallel compile fail when libbitcoin_server objects start before
# obj/build.h is generated by genbuild.sh. The build dir for that
# header (src/obj/) doesn't exist yet at compile-time on a clean tree,
# so clang's -I./obj is treated as a missing-dir error. Pre-creating
# src/obj/ resolves the race deterministically.
...
# Inject 'mkdir -p src/obj' immediately after ./configure in the build
# script. Find the configure line and add the mkdir after it.
script = d.get('script', '')
needle = 'CONFIG_SITE=\${BASEPREFIX}/\${i}/share/config.site ./configure'
patch_line = '    mkdir -p src/obj # zcash-gitian: pre-create dir for -I./obj (Wmissing-include-dirs is fatal)'
patched = []
inserted = False
for line in script.split('\n'):
    patched.append(line)
    if not inserted and needle in line:
        patched.append(patch_line)
        inserted = True
if not inserted:
    raise SystemExit('ERROR: could not find ./configure line in gitian descriptor to patch')
d['script'] = '\n'.join(patched)
with open('$GITIAN_DESC', 'w') as f:
    yaml.dump(d, f, default_flow_style=False, sort_keys=False)
print('Suite: $SUITE, cache: disabled, src/obj pre-create patch: applied')
"
