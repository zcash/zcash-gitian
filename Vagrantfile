...
Vagrant.configure(2) do |config|
  ...
  config.vm.define 'zcash-build', autostart: false do |gitian|
    ...
    gitian.vm.provision "shell", inline: <<-SHELL
      ...
      sudo DEBIAN_FRONTEND=noninteractive apt install bridge-utils curl gpg sudo ruby qemu-utils python3-cheetah make lxc lintian kpartx gnupg2 git-core debootstrap debian-archive-keyring apt-cacher-ng -y
